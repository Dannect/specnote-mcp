...
**v0.3.0 — Dev/Prod env substitution 폐기** (이전 "Failed to connect" 사고 영구 차단 · prod URL hardcode). 첫 호출 시 browser 자동 로그인 → 토큰 자동 회전 → 30일간 재로그인 불필요.
...
Plugin 의 prod URL 은 `https://specnote.io/api/mcp/mcp` 로 **고정** (v0.3.0 — env substitution 폐기 · 이전 "Failed to connect" 사고 영구 차단).

Specnote 본체를 로컬에서 개발할 때 (`pnpm dev`) 는 **별도 MCP 서버를 수동 등록**:

```bash
claude mcp add specnote-dev http://localhost:3000/api/mcp/mcp --transport http
```

이렇게 두 개의 MCP server 가 공존:
- `specnote` (plugin · prod) — `https://specnote.io/api/mcp/mcp`
- `specnote-dev` (수동 등록 · dev) — `http://localhost:3000/api/mcp/mcp`

자연어 호출 시 Claude Code 가 두 서버 모두에 동일 tool 노출 (이름 동일이지만 라우팅은 사용자 의도/context 로 분기 · 또는 `/mcp` 메뉴에서 명시).

---

## 환경변수 사양

**v0.3.0 부터 환경변수 0개** — Plugin 은 prod URL hardcode + OAuth 2.1 자동 토큰. 이전 `SPECNOTE_URL` · `SPECNOTE_TOKEN` 모두 폐기.

Dev 환경은 위 "Dev / Prod 분리" 의 수동 등록 명령으로 처리.
...
### `Failed to connect`
- prod URL 정상: `curl -i https://specnote.io/api/mcp/mcp` → 401 + `WWW-Authenticate: Bearer ...` (auth 살아있음)
- dev 사용 시 specnote 서버 띄워졌는지: `curl http://localhost:3000/api/mcp/mcp` → 401
- OAuth 자동 flow 진입 안 됨 → Claude Code 재시작 후 재시도 (`~/.claude.json` 안 stale OAuth state 정리되면서 새 flow 시작)
- v0.2.0 이전 사용자: `~/.zshrc` 의 `SPECNOTE_URL` env 라인 제거 후 새 터미널 + Claude 재시작
...
v0.2.0 의 `${SPECNOTE_URL:-...}` env substitution 이 "Failed to connect" 사고를 유발했음. v0.3.0 에서 prod URL hardcode 로 영구 차단.

v0.2.0 사용자라면:
1. `~/.zshrc` 등의 `SPECNOTE_URL` env 라인 제거 (있다면)
2. Plugin 업데이트 — Claude Code 안에서 `/plugin update specnote@specnote-mcp`
3. Dev 환경은 위 "Dev / Prod 분리" 섹션의 수동 등록 명령으로 처리

## v0.1.0 → v0.3.0 Migration

v0.1.0 사용자라면:
1. `~/.zshrc` 또는 `~/.zshenv` 의 `SPECNOTE_TOKEN` · `SPECNOTE_URL` env 라인 모두 제거
2. Specnote 마이페이지의 옛 PAT (`spnt_*`) 폐기 (선택 · OAuth 로 교체)
3. Plugin 업데이트 — Claude Code 안에서 `/plugin update specnote@specnote-mcp`
4. 다음 MCP 호출 시 browser 자동 OAuth
